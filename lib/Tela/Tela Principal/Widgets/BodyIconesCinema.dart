...
class BodyIconesCinema extends StatefulWidget {
  final PrincipalCtrl ctrl;
  final double tamanhoBloco;

  const BodyIconesCinema({
    super.key,
    required this.ctrl,
  ...
  @override
  State<BodyIconesCinema> createState() => _BodyIconesCinemaState();
}
...
class _BodyIconesCinemaState extends State<BodyIconesCinema> {
  ...
  Widget build(BuildContext context) {
    return FocusScope(
      node: widget.ctrl.focusScopeCinema,
      child: Container(
        ...
        child: GridView.builder(
          ...
          itemCount: widget.ctrl.listCinema.length,
          ...
          itemBuilder: (context, index) {
            bool foco = index == widget.ctrl.selectedIndexCinema;
  ...
  Widget _buildMediaButton(int i, bool foco) {
    ...
    return Focus(
      focusNode: widget.ctrl.focusNodeCinema[i],
      onFocusChange: (hasFocus) {
        if (hasFocus) {
          widget.ctrl.onFocusChangeGrid(i, PrincipalCtrl.cine);
        }
      },
      child: FittedBox(
        child: Container(
          decoration: BoxDecoration(
            image: DecorationImage(
              fit: BoxFit.cover,
              image: FileImage(File(widget.ctrl.listCinema[i].imgLocal)),
            ),
            border: foco
                ? Border.all(
                    color: Colors.white,
